...
# global
_BATCH_ACCOUNT_NAME = '{{cookiecutter.batch_account_name}}'
_BATCH_ACCOUNT_KEY = '{{cookiecutter.batch_account_key}}'
_BATCH_ACCOUNT_URL = '{{cookiecutter.batch_account_url}}'

_STORAGE_ACCOUNT_NAME = '{{cookiecutter.storage_account_name}}'
_STORAGE_ACCOUNT_KEY = '{{cookiecutter.storage_account_key}}'
...
def upload_file_to_datalake(datalake_client, file_path):
    """
    Uploads a local file to Azure Blob Datalake.

    :param datalake_client: A datalake service client.
    :param str file_path: The local absolute path to the file.
    """
    datalake_path = "/" + os.path.basename(file_path)
    print('Uploading file {} to datalake {}'.format(file_path, datalake_path))
    multithread.ADLUploader(datalake_client, lpath=file_path, rpath=datalake_path, nthreads=64, overwrite=True, verbose=False)
    pass
...
def add_tasks(batch_service_client, job_id, input_file_paths,
    ...
    :param list input_file_paths: A collection of input file path. One task will be
    ...
    print('Adding {} tasks to job [{}]...'.format(len(input_file_paths), job_id))

    tasks = list()

    for idx, input_file_path in enumerate(input_file_paths):

        datalake_path = "/" + os.path.basename(input_file_path)
...
if __name__ == '__main__':
    ...
    # Upload the data files to Azure Datalake. This is the data that will be processed by each of
    # the tasks executed on the compute nodes in the pool.
    [upload_file_to_datalake(datalake_client, file_path) for file_path in input_file_paths]
